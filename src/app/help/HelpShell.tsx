...
export default function HelpShell({
  ...
  return (
    ...
    >
      ...
      {/* Help grid with sticky sidebar */}
      <div className="mx-auto w-full max-w-[1400px] px-4 sm:px-6 py-6">
        <div
          className="
            grid min-h-0 gap-x-4 overflow-visible
        ...
        >
          ...
          {/* RIGHT: Route content (only scroller) */}
          <main className="min-h-0 min-w-0">
            <div
              className="
                h-[calc(100vh-var(--help-header-h)-48px)]
                overflow-y-auto overscroll-y-contain
                rounded-lg border border-border bg-card text-card-foreground shadow-sm
              "
            >
              {/* Sticky breadcrumbs */}
              <div className="sticky top-0 z-30 bg-card border-b border-border px-5 pt-5 sm:px-6">
                <Breadcrumb />
              </div>

              {/* Sticky title + description */}
              <div className="sticky top-[56px] z-20 bg-card border-b border-border px-5 sm:px-6 py-4">
